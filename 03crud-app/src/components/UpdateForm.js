...
class UpdateForm extends Component {
  ...
  */
  ...
  constructor(props) {
    ...
    //state는 게시물 수정을 위해 필요한 일련번호, 제목, 내용으로 구성된다.
    this.state = {
  ...
  */
  ...
  render() {
    return (
      <article>
        <h2>Update</h2>
        {/* 리액트에서는 action과 method가 사실상 의미가 없다. */}
        <form
          action="/create_process"
          method="post"
          onSubmit={(e) => {
            e.preventDefault();
            //부모에서 전달할 props를 통해 폼값을 전송한다. 즉 부모 컴포넌트로 전달한다.
            this.props.onSubmitValue(
              //값 전송시 이벤트 객체를 사용하고 있다.
              e.target.id.value,
        ...
        >
          ...
          방식을 사용하면 React가 props로 인식하여 수정이 불가낭한 상태가 된다.
          즉 "value=기존값" 과 같이 사용할 수 없다. */}

          {/* 부모 컴포넌트인 App.js에서 기존 게시물을 props로 전달한 후 input 상자에
          즉시 삽입하면, 기존 게시물을 출력할수는 있으나 역시나 수정은 불가능하다.
