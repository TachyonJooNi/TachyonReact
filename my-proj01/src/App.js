  ...
  render() {
    ...
    */
    if (this.state.mode === "welcome") {
      ...
      _article = (
        <CreateForm
          onSubmitValue={function (_title, _desc) {
            //전달받은 폼값을 로그로 출력한다.
            ...
            concat() 함수는 배열을 추가하여 새로운 배열을 만드는 기능을 가지고있다.
            --K11Reactjs -> 01ES6 -> ES6-06Array.html 교안에서 다뤄보았다.
            */
      ...
      */
      ...
      while (i < this.state.contents.length) {
        ...
        //현재 조회중인 게시물 번호(selected_content_id)와 배열에 저장된 id값이 일치하면..
      ...
      //게시물 수정을 위한 수정폼 컴포넌트 추가. 현재 조회중인 게시물을 props로 전달한다.(readData)
      //수정폼에서 작성한 폼값을 전달받기 위한 props도 전달한다. (onSubmitValue)
      _article = (
        <UpdateForm
          readData={_readData}
          onSubmitValue={(_id, _title, _desc) => {
            ...
            // _contents[this.state.selected_content_id - 1] = { -> 인덱스에 문제있음
            //   id: Number(_id),
            //   title: _title,
            //   desc: _desc,
            // };
            ...
            //변경된 배열을 state에 적용한 후 렌더링한다. 특히 mod를 read로 변경하여
            //수정된 내용을 즉시 확인할 수 있다.
            this.setState({
              contents: _contents,
              mode: "read",
      ...
      <div className="App">
        ...
        <Buttons
          onChangeMode={(btn_mode) => {
            if (btn_mode === "delete") {
              //리액트에서는 confirm() 사용시 window를 반드시 붙여야한다.
              //if (confirm("삭제할까요?")) { -> 에러발생 (자바스크립트에서는 생략가능하지만, 리액트에선 X )
              ...
              /*
              게시물을 삭제하면 더이상 read 할수 없으므로 welcome으로 변경한다.
