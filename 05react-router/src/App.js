/*
react-router-dom
  : React는 기본적으로 화면의 새로고침 없이 화면을 갱신한다. 이 경우 주소가
...
BrowserRouter
  : 리액트 라우터 돔을 적용하고 싶은 컴포넌트의 최상위 컴포넌트를 감싸주는
  Wrapper(래퍼)로 사용한다.
Route
...
-- useParams, Outlet, useLocation, useSearchParams 를 추가적으로 사용해보았다.
*/
  ...
  return (
    <BrowserRouter>
      <div className="App">
        ...
        <h4>a태그 : 화면의 깜빡임 있음</h4>
        <ul>
          <li>
            <a href="/">Home(a태그)</a>
          </li>
          <li>
            <a href="/Topics">Topics(a태그)</a>
          </li>
          <li>
        ...
        <h4>Link태그 : 화면의 깜빡임 없음(Style없음)</h4>
        <ul>
          <li>
            <Link to="/">Home</Link>
          </li>
        ...
        <h4>NavLink태그 : 화면의 깜빡임 없음(Style추가됨)</h4>
        <ul>
          <li>
          ...
          <li>
            <NavLink to="/Abcd1234">없는링크</NavLink>
          </li>
        </ul>
        {/* 
...
function Topics() {
  ...
  return (
    <div>
      ...
      <ul>
        {/* <li>
          <NavLink to="/Topics/free">자유게시판</NavLink>
        </li>
        <li>
          <NavLink to="/Topics/qna">질문과답변</NavLink>
        </li>
        <li>
          <NavLink to="/Topics/faq">자주묻는질문</NavLink>
        </li> */}
        {menuArray}
      </ul>
      {/* <Outlet>을 사용하지 않는 경우 중첩 처리 */}
      {/* <Desc /> */}
      {/* 중첩 라우팅을 사용하는 경우 자식 컴포넌트로 선언된 객체를 렌더링한다. */}
      <Outlet />
    </div>
  );
}
...
function Desc() {
  //리액트 훅에서 제공하는 함수로 파라미터 정보를 얻어올 수 있다.
