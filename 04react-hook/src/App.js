...
function FuncComponent(props) {
  ...
  */
  ...
  /*
  해당 함수형 컴포넌트가 렌더링 된 후(리턴이 되고난 후) 자동으로 호출된다. 해당 함수는
  첫번째 인자로 반드시 무기명함수를 사용해야 한다.
  클래스형 컴포넌트는 마운팅단계와 업데이트단계에서 사용하는 별도의 수명주기 함수가 있지만
  함수형에서는 별도로 존재하지 않는다.
  */
  useEffect(function () {
    console.log("#Life3#", "FuncComponent ==> userEffect");
  });
  ...
  */
  ...
  return (
    <div className="container">
      ...
      즉, 버튼을 클릭할때마다 생성된 난수로 state를 변경한다. */}
      <input
        type="button"
        value="난수생성"
        onClick={function () {
          setNumber(Math.random());
          /* 일반적인 함수에서 bind() 할 필요 없음. */
        }}
      />

      {/* 함수형 컨포넌트에서는 state값을 변경하기 위해 this를 사용할 필요가 없으므로
...
class ClassComponent extends React.Component {
  ...
  클래스형 컴포넌트에서 2개 이상의 state가 필요한 경우 컴마로 구분하여 추가하면된다.
  */
  state = {
  ...
  전달된 props를 통해 state를 변경하는 역할을 한다. 또한 반드시 반환값이 있어야한다.
  */
  ...
  render() {
    return (
      <div className="container">
        ...
        {/* 클래스형 컴포넌트에서 일반 함수를 사용할때는 반드시 this와 bind()해줘야
         한다. 하지만 화살표 함수(Arrow function)을 사용하면 별도의 바인딩이 필요없다. */}
        <input
          type="button"
          value="현재날짜"
          onClick={() => {
