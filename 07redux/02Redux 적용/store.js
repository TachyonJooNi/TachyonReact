...
리듀서(Reducer)
  - 변화를 일으키는 함수를 말한다.
  - state(현재의 상태), action(전달받은 액션) 두가지의 파라미터를 받아온다.
디스패치(Dispatch)
  - 스토어의 내장함수로 액션을 발생시킨다.
  - 액션과 타입 등을 매개변수로 전달한다.
구독(Subscribe)
  - 스토어의 내장함수로 함수 형태의 값을 파라미터로 받아온다.
  - subscribe 함수에 특정 함수를 전달하면, 액션이 디스패치 될때마다 전달해준
  함수가 호출된다.

--getState() 를 통해 store 의 데이터를 꺼낼수있다. 
*/

//createStore 모듈 임포트. (legacy_createStore : 현재 이상하게 사용중지 권고상태에 있어서 넣어줬다.)
...
export default createStore(function (state, action) {
  //만약 state가 undefined 이라면(정의된게 없다면) number를 0으로 초기화한다.
  if (state === undefined) {
    return { number: 0 };
  }
